...
class Process {
    ...
    public double  getResponseRatioTime(int cTime)  { return (float)(getWaitingTime(cTime) + serviceTime) / serviceTime; }

    // 프로세스의 이름을 반환함
    public String  getName()              { return name; }

    public void println(int cTime) {
        System.out.printf("%s: s(%d) e(%d) r(%d) w(%d) rr(%5.2f) f(%s)\n",  
            name, getServiceTime(), executionTime, getRemainingTime(), 
            getWaitingTime(cTime), getResponseRatioTime(cTime), isFinished());
    }
    public String toString() {
        return String.format("%s: a(%d) s(%d) e(%d)", 
