...
class UI {
	...
	static void nexts(String arr[]) {
		for (int i = 0; i < arr.length; ++i) {
			arr[i] = scan.next();
			if (echo_input)
				System.out.print(arr[i] + " "); // 자동 오류 체크시 출력
		} // 자동 오류 체크시 출력
		if (echo_input)
			System.out.println();
	}
...
class Process {
	...
	public boolean isFinished() // return executionTime >= serviceTime;로도 가능
	{
		if (executionTime >= serviceTime) // 실행 끝
		{
			return true;
		} else // 실행 남아있음
		{
	...
	public double getResponseRatioTime(int cTime) {
		return (float) (getWaitingTime(cTime) + serviceTime) / serviceTime;
	}
	...
	public void println(int cTime) {
		System.out.printf("%s: s(%d) e(%d) r(%d) w(%2d) rr(%5.2f) f(%s)\n", name, getServiceTime(), executionTime,
				getRemainingTime(), getWaitingTime(cTime), getResponseRatioTime(cTime), isFinished());
	}
	...
	class SPN extends Scheduler{ 
		...
		public void schedule() {
		    ...
		    if (!readyQueue.isEmpty()) {
		        // readyQueue의 첫 번째 프로세스를 초기값으로 설정
		        Process shortProcess = readyQueue.getFirst();

		        // 나머지 프로세스와 비교하여 서비스 시간이 가장 짧은 프로세스를 찾음
		        for (Process p : readyQueue) {
		        ...
		        currentProcess = null; // readyQueue가 비어 있으면 null로 설정
		    }
		}
	}
...
// 이 클래스의 구현 시
//    1) 추상 클래스 Scheduler를 상속받아야 한다. (SPN 클래스 참조)
//    2) 생성자 메소드를 정의하라. (SPN 클래스 참조)
//    3) Scheduler 클래스의 schedule() 메소드를 재정의하라. (SPN 클래스 참조)
//       이 메소드 구현 시
//          먼저 슈퍼 클래스의 schedule()를 먼저 호출
//          for문(SPN 참조)을 이용하여 readyQueue에 있는 프로세스들 중  
...
//-----------------------------------------------------------------------------
	...
	class HRRN extends Scheduler { 
	    ...
	    public void schedule() {
	        ...
	        if (!readyQueue.isEmpty()) {
	            Process bestProcess = readyQueue.getFirst();
	            double highestResponseRatio = -100;
	            
	            for (Process p : readyQueue) {
	                // bestprocess 찾음.
	                if (p.getResponseRatioTime(currentTime) > highestResponseRatio) {
	                    highestResponseRatio = p.getResponseRatioTime(currentTime);
	                    bestProcess = p;
	                }
	            }
	            currentProcess = bestProcess;
	        } else {
	            currentProcess = null; // readyQueue가 빔 -> null
	        }
	    }
	}
...
//	          새로 도착한 프로세스가 있는 경우 true를 반환 (리턴문을 한 문장으로 코딩해 보라)
	...
	class SRT extends Scheduler {
	    ...
	    public void schedule() {
	        ...
	        if (!readyQueue.isEmpty()) {
	            Process shortProcess = null; // 가장 짧은 남은 실행 시간을 가진 프로세스를 저장할 변수
	            int stRemainTime = Integer.MAX_VALUE; // 초기값은 최대값

	            // 가장 작은 Remaining Time을 가진 프로세스 찾기
	            for (Process p : readyQueue) {
	                if (p.getRemainingTime() < stRemainTime) {
	                    stRemainTime = p.getRemainingTime();
	                    shortProcess = p;
	                }
	            }

	            currentProcess = shortProcess; // 선택된 프로세스를 현재 실행 프로세스로 설정
	        } else {
	            currentProcess = null; // readyQueue가 비어 있는 경우
	        }
	    }

	    @Override
	    public boolean isSchedulable() {
	        // 새로 도착한 프로세스가 있는 경우나 기존 프로세스가 종료된 경우 true 반환
...
//    1) 추상 클래스 Scheduler를 상속받아야 한다. (FCFS 클래스 참조)
//    2) 생성자 메소드를 정의하라.   
//    3) 위 timeQuantumExhausted() 메소드를 정의하라.   
//    4) Scheduler 클래스의 schedule() 메소드를 재정의하여 override하라. (FCFS 클래스 참조)
//       이 메소드 구현 시
//          먼저 슈퍼 클래스의 schedule()를 먼저 호출
//          현재 실행하는 프로세스가 주어진 time quantum을 모두 소진한 경우(timeQuantumExhausted())
...
//          먼저 슈퍼 클래스의 isSchedulable()를 먼저 호출하여 스케줄링 가능한 경우 또는
//          주어진 time quantum을 모두 소진한 경우(timeQuantumExhausted())
 ...
 */
	 class RR extends Scheduler {
		    ...
		    //5
		    @Override
		    public boolean isSchedulable() { return super.isSchedulable() || timeQuantumExhausted(); }
...
class MainMenu {
	public static void run() {
		...
		while (true) {
			...
			case 3:
				cs.run(jobs.new FCFS("FCFS"));// cs에 스케줄러를 설정하고 스케줄링을 시작한다.
				break;
				/******************************************************************************
				 * 문제 5-2
				 ******************************************************************************/
				// MainMenu::run() 메소드에서 메인 메뉴의 4번 항목을 실행하는 switch 문장에 추가하라.
				// 4번 항목이 선택된 경우 SPN 클래스의 객체를 생성한 후 이를 인자로 사용하여 cs.run()을 호출하라.
				// SPN 스케줄러 객체 생성 시 스케줄러 이름은 "SPN"으로 하라.
				// 3번 메뉴 항목(FCFS)을 실행하는 case 문장을 참조하라. 
				//-----------------------------------------------------------------------------
			case 4:
				cs.run(jobs.new SPN("SPN"));
				break;
				/******************************************************************************
				 * 문제 6-2
				 ******************************************************************************/
				// MainMenu::run() 메소드에서 메인 메뉴의 5번 항목을 실행하는 switch 문장에 추가하라.
				// 5번 항목이 선택된 경우 HRRN 클래스의 객체를 생성한 후 이를 인자로 사용하여 cs.run()을 호출하라.
				// HRRN 스케줄러 객체 생성 시 스케줄러 이름은 "HRRN"으로 하라.
				// 4번 메뉴 항목을 실행하는 case 문장을 참조하라. 
				//-----------------------------------------------------------------------------
			case 5:
				cs.run(jobs.new HRRN("HRRN"));
				break;
				/******************************************************************************
				 * 문제 7-2
				 ******************************************************************************/
				// MainMenu::run() 메소드에서 메인 메뉴의 6번 항목을 실행하는 switch 문장에 추가하라.
				// 6번 항목이 선택된 경우 SRT 클래스의 객체를 생성한 후 이를 인자로 사용하여 cs.run()을 호출하라.
				// SRT 스케줄러 객체 생성 시 스케줄러 이름은 "SRT"으로 하라.
				// 5번 메뉴 항목을 실행하는 case 문장을 참조하라. 
				//-----------------------------------------------------------------------------
			case 6:
				cs.run(jobs.new SRT("SRT"));
				break;
				/******************************************************************************
				 * 문제 8-2
				 ******************************************************************************/
				// MainMenu::run() 메소드에서 메인 메뉴의 
				// 1) 7번 항목을 실행하는 switch 문장에 추가하라.
//				    7번 항목이 선택된 경우 RR 클래스의 객체를 생성한 후 이를 인자로 사용하여 cs.run()을 호출하라.
//				    RR 스케줄러 객체 생성 시 스케줄러 이름은 "RR(q=1)"으로 하고, time quantum으로 1을 지정하라.
				//-----------------------------------------------------------------------------
				// 2) 또한 8번 항목을 실행하는 case 문장을 기존 switch 문장에 추가하라.
//				    8번 항목이 선택된 경우 RR 클래스의 객체를 생성한 후 이를 인자로 사용하여 cs.run()을 호출하라.
//				    RR 스케줄러 객체 생성 시 스케줄러 이름은 "RR(q=4)"로 하고, time quantum으로 4를 지정하라.
				//-----------------------------------------------------------------------------
			case 7:
				cs.run(jobs.new RR("RR(q=1)", 1));
				break;
			case 8:
				cs.run(jobs.new RR("RR(q=4)", 4));
				break;
			default:
	...
	class ComputerSystem {
		...
		// 문제 3:
		public void printEpilog(Jobs.Scheduler scheduler) {
			/*
		...
		public void run(Jobs.Scheduler scheduler) { // 스케줄링 알고리즘을 테스트 함
			...
			while (scheduler.hasMoreProcessesToExecute()) { // 아직 더 실행해야 할 프로세스가 있는지 체크
				...
				try { // 우리 스케줄러에서 사용할 시간단위는 100ms: 빠르게 실행하려면 이 값을 10 또는 1로 줄여도 됨
					...
					Thread.sleep(0); // 100 millisecond 동안 정지했다가 리턴함 -> 수정
				}
